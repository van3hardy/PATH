...
import { sendLinkedInMessage } from '../../transports/linkedin-send.mjs';

const RELAY_URL = 'https://relay.example.test/linkedin';
...
test('happy path posts JSON payload to the relay and returns its messageId', async () => {
  ...
  const fetchFn = async (url, init) => {
    calls.push({ url, init });
    return { ok: true, status: 200, json: async () => ({ messageId: 'li-123' }) };
  };

  const result = await sendLinkedInMessage({ ...DEFAULT_ARGS, fetchFn });
  assert.deepEqual(result, { ok: true, messageId: 'li-123' });
  ...
  assert.deepEqual(payload.to, { name: 'Hiring Manager', address: 'hm@example.com' });
  assert.equal(payload.subject, 'AI Engineer @ Example Company');
  assert.equal(payload.body, 'Agent workflows on Windows 11.');
});
...
test('api key is sent as a bearer header when provided', async () => {
  ...
  const fetchFn = async (url, init) => {
    calls.push({ url, init });
    return { ok: true, status: 200, json: async () => ({ messageId: 'li-key' }) };
  };

  const result = await sendLinkedInMessage({ ...DEFAULT_ARGS, apiKey: 'secret-key', fetchFn });
  assert.deepEqual(result, { ok: true, messageId: 'li-key' });
...
test('no authorization header is sent without an api key', async () => {
  ...
  const fetchFn = async (url, init) => {
    calls.push({ url, init });
    return { ok: true, status: 200, json: async () => ({ messageId: 'li-noauth' }) };
  };

  await sendLinkedInMessage({ ...DEFAULT_ARGS, fetchFn });
...
test('missing relay URL fails closed with SEND_FAILED_CONFIG before any network call', async () => {
  ...
  await assert.rejects(
    sendLinkedInMessage({ ...DEFAULT_ARGS, relayUrl: undefined, fetchFn }),
...
for (const status of [400, 401, 403, 429, 500]) {
  test(`relay ${status} maps to SEND_FAILED_API`, async () => {
    ...
    await assert.rejects(
      sendLinkedInMessage({ ...DEFAULT_ARGS, fetchFn }),
