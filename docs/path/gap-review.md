...
| §6 Professional network engine | ⚠️ | `modes/contacto.md`, `followup-cadence.mjs` `extractContacts()`, `invite-match.mjs`, `path-workflows/recruiter/request-boundary.mjs`, `path-safety/contacts.mjs` + `data/contacts.jsonl` | **People ledger + read-only web surface + derived graph edges + channel-scoped dedup + name-only dedup.** Contacts are seeded (`scripts/contacts-backfill.mjs`, now including name-only contacts) and deduped by person via `contactId` in the dispatch gate; `/api/contacts` (`09c0bc5`) exposes the ledger read-only, plus an additive `graph` block (edges resolved to company/role from the tracker, per-company distinct-person counts, per-person summaries). The dispatch gate blocks per-channel (a prior email blocks email but allows LinkedIn), canonicalizing `gmail`→`email`, and additionally matches name-only recipients by exact canonical name (`isContactedByNameOnChannel`). |
...
| §9 Conversation intelligence | ✅ | `reply-watch.mjs`, `reply-matcher.mjs`, `paste-reply.mjs`, `invite-match.mjs`, `gmail-scan-replies.mjs`, `data/reply-candidates.json` | Recognize/respond/record machinery is local and complete. Feed is no longer manual-only: `gmail-scan-replies.mjs` (standalone, `#1583` closed) scans `in:inbox` into `data/reply-candidates.json` (shared `data/gmail-state.json` cursor, blocklist, `--dry-run`). Classification stays in `reply-watch.mjs`. |
...
| 3 | **Career truth store** | ✅ | `verify-cv-facts.mjs`, `fact-resolver.mjs`, `evidence-selector.mjs`. Approved-fact store populated (`config/path.facts.yml`, 7 facts incl. career facts); `verify-cv-facts.mjs` consults the store (`--facts`, `factsPath`) as an external authority for approved metrics + non-metric facts; real `config/cv-facts.json` created. Design: `2026-08-14-career-truth-store-design.md`. |
| 4 | **CV/application engine** | ✅ | `build-cv-*`, `generate-pdf/latex`, `prepare-application`, `application-artifacts`. |
| 5 | **Contact graph** | ✅ | People ledger ships: `path-safety/contacts.mjs`, `data/contacts.jsonl`, `--contacts` gate in `scripts/path-dispatch.mjs`, read-only web surface `/api/contacts` + `web/src/lib/contact-graph.mjs` (`09c0bc5`), derived edges (company/role/timeline) returned as additive `graph` block (`4bfd3c1`), channel-scoped dedup (`isContactedOnChannel`, same-channel blocks / cross-channel allowed, canonical write-back), name-only dedup (`findPersonByName` + `isContactedByNameOnChannel`, distinct `c-n-` identity namespace, backfill seeds name+channel records), `docs/path/contact-graph.md`. |
...
The repo is the vision's **job-seeker core, fully and maturely built**, plus a **hardened human-in-the-loop recruiter MVP-1** that stops at review. Everything downstream of "prepared draft + approved" (§7 communications, real LinkedIn/call/email, PATH Brain, learning loop, deployment) is genuinely **not yet built** — matching §17 of the vision honestly.

The gaps are **not empty code**: the machinery (approval packets, outboxes, claim checks, versioned artifacts, tracker, contact ledger + derived graph) is real and tested. The core missing piece across the remaining vision items is still **transport** — with the people ledger and graph foundations now in place for the contact graph.