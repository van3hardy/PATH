...
| §6 Professional network engine | ⚠️ | `modes/contacto.md`, `followup-cadence.mjs` `extractContacts()`, `invite-match.mjs`, `path-workflows/recruiter/request-boundary.mjs`, `path-safety/contacts.mjs` + `data/contacts.jsonl` | **People ledger + read-only web surface + derived graph edges + channel-scoped dedup.** Contacts are seeded (`scripts/contacts-backfill.mjs`) and deduped by person via `contactId` in the dispatch gate; `/api/contacts` (`09c0bc5`) exposes the ledger read-only, plus an additive `graph` block (edges resolved to company/role from the tracker, per-company distinct-person counts, per-person summaries). The dispatch gate now blocks per-channel (a prior email blocks email but allows LinkedIn), canonicalizing `gmail`→`email`. Dedup on name-only contacts still deferred — see [`contact-graph.md`](contact-graph.md). |
