...
| §6 Professional network engine | ⚠️ | `modes/contacto.md`, `followup-cadence.mjs` `extractContacts()`, `invite-match.mjs`, `path-workflows/recruiter/request-boundary.mjs` | **No people/contact data.store.** Contacts are strings extracted from tracker notes; no graph, no "prevent duplicate contact". |
...
| §10 CRM & follow-up | ⚠️ | `data/applications.md`, `tracker*.mjs`, `merge/dedup/reconcile-pipeline.mjs`, `followup-cadence.mjs`, `followup-seed.mjs`, `reply-watch.mjs`, `agent-inbox.mjs` | Application CRM is mature. **Partial:** no people database, so "prevent duplicate contact" is only per-application; deadlines computed on demand, not pushed. |
...
| 5 | **Contact graph** | ❌ | No people database. Only ad-hoc strings (`followup-cadence.mjs` `extractContacts()`). |
...
5. **No people/contact database anywhere** — only strings; "prevent duplicate contact" is per-application, not per-person.
...
The gaps are **not empty code**: the machinery (approval packets, outboxes, claim checks, versioned artifacts, tracker) is real and tested. The core missing piece across every remaining vision item is **transport + a people data model** — that's the natural next build.