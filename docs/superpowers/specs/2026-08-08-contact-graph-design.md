...
- Dedup on name-only contacts (no email yet) → SHIPPED. Records without an email carry a distinct `c-n-` contactId; backfill seeds them; the gate matches a no-email recipient by exact canonical name with the same channel scoping (`isContactedByNameOnChannel`). See plan `2026-08-09-name-only-dedup.md`.