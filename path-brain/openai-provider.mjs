...
import { DEFAULT_OPENAI_MODEL } from './provider-ids.mjs';
...
export function createOpenAIProvider(options = {}) {
  ...
  if (typeof model !== 'string' || model.trim().length === 0) {
    throw codedError('INVALID_OPENAI_PROVIDER_OPTIONS');
  }
...
async function defaultTransport({ model, apiKey, prompt }, { signal } = {}) {
  if (!isNonemptyString(apiKey)) throw codedError('BLOCKED_NO_OPENAI_KEY');
  ...
  if (!response.ok) {
    throw Object.assign(new Error('FAILED_BRAIN_PROVIDER'), {
      ...
      cause: `OpenAI HTTP ${response.status}`
    });
  }
  let data;
  try {
...
function resolveApiKey(explicitKey) {
  ...
  if (isNonemptyString(process.env.OPENAI_API_KEY)) return process.env.OPENAI_API_KEY.trim();
  return null;
}
